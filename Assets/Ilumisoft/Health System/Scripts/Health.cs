...
namespace Ilumisoft.HealthSystem
{
    ...
    public class Health : HealthComponent
    {
        ...
        private float drainRate; // health per second
        ...
        private void Start()
        {
            // Calculate how fast health should decrease per second
            drainRate = MaxHealth / drainDuration;

            // Start health drain when the game starts
        ...
        private IEnumerator DrainHealthOverTime()
        {
            while (IsAlive)
            {
                // decrease health gradually
                ApplyDamage(drainRate * Time.deltaTime);
